...
@section('content')
    <section class="pt-3 pb-1 mb-2 border-bottom">
        <h1 class="h5">Edit Ticket Admin #{{ $TicketAdmin->id }}</h1>
    </section>
    <section class="row my-3">
        <section class="col-12">
            <form method="post" action="{{ route('admin.tickets-admins.update',$TicketAdmin->id) }}">
                @csrf
                @method('PUT')
                <div class="form-group">
                    ...
                    <select name="user_id" id="user_id" class="form-control" required autofocus>
                        ...
                        @foreach ($users as $user)
                            <option value="{{ $user->id }}" @if ($TicketAdmin->user_id == $user->id) selected @endif>
